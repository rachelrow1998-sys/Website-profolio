...
- ✨ **动效全手写，没有引入 GSAP / Lenis**（省 70KB，网站更快 —— 速度本身就是你的卖点）
  惯性平滑滚动、滚动速度倾斜、视差、标题字符逐个入场、磁吸按钮、卡片 3D 倾斜
...
**照片**：存成 `assets/img/me.jpg`，显示在最后的「联系我」那一栏，是个圆形头像。
建议正方形、背景干净、脸在正中间（约 800 × 800）。没放也不会破图 —— 会显示一个占位剪影。
...
⚠️ **只有默认分支会发布。** `github-pages` 这个环境只允许默认分支部署，
所以 workflow 监听的就是默认分支 —— 目前是 `claude/online-portfolio-website-2x0nbr`
（不是 `main`；推 `main` 的部署会在第一步就被挡掉）。在别的分支上改完，
记得合并回默认分支，别的分支不会发布。

想改成 `main`：Settings → General → Default branch 换成 `main`，
再把 `.github/workflows/deploy-pages.yml` 里监听的分支名一起改掉。
...
**手机上会自动关掉**平滑滚动、磁吸和 3D 倾斜（触屏上这些只会碍事）。
